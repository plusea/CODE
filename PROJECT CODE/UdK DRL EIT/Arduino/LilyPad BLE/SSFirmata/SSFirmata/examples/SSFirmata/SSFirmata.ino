...
/*
  ...
  Copyright (C) 2013 Martijn & Juan @ EIT. Rock n roll.
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  case SAMPLING_INTERVAL:
    if (argc > 1) {
      ...
      //SSFirmata.sendString("Not enough data");
    }
    break;
...
void setup() 
{
  ...
  SSFirmata.attach(DIGITAL_MESSAGE, digitalWriteCallback);
  SSFirmata.attach(REPORT_ANALOG, reportAnalogCallback);
  SSFirmata.attach(REPORT_DIGITAL, reportDigitalCallback);
  SSFirmata.attach(SET_PIN_MODE, setPinModeCallback);
  SSFirmata.attach(START_SYSEX, sysexCallback);
  SSFirmata.attach(SYSTEM_RESET, systemResetCallback);
    ...
    for(pin=0; pin<TOTAL_PINS; pin++) {
      if (IS_PIN_ANALOG(pin) && pinConfig[pin] == ANALOG) {
        ...
        if (analogInputsToReport & (1 << analogPin)) {
          SSFirmata.sendAnalog(analogPin, analogRead(analogPin));
        }
      }
    }
