...
void loop()
{
  ...
  if(mode == 0) {
    strip.setPixelColor(0, strip.Color(potValue,potValue,0));
    strip.show();
  }

  if(mode == 1) {
    strip.setPixelColor(1, strip.Color(potValue,potValue,0));
    strip.show();
  }

  if(mode == 2) {
    strip.setPixelColor(2, strip.Color(potValue,potValue,0));
    strip.show();
  }
  ...
  if(mode == 6) {
    ...
    for (int i = 0; i< textlength;i++){
      ...
      activeGroup.setRGB(potValue, 0, 0);
      writeGroups();
      delay(1000);
    }
  }
